...
class WidgetDetectorState extends State<WidgetDetector> {
  ...
  void _hitTest(List<_RenderObjectSelection> selections, Offset position,
      RenderBox object) {
    ...
    for (int i = 0; i < hitTestEntries.length; i++) {
      ...
      if (_checkElement(ele)) {
        selections.add(_RenderObjectSelection(
            renderObject: testEntry.target, element: ele));
      }
      ...
      ele.visitAncestorElements((Element ancestor) {
        if (nextTestEntry == null ||
            ancestor.runtimeType ==
                nextTestEntry.target.debugCreator.element.runtimeType)
          return false;

        if (_checkElement(ancestor)) {
          selections.add(_RenderObjectSelection(
              renderObject: testEntry.target, element: ancestor));
          return true;
        }
        return false;
      });
...
//    String widgetType = ele.widget.runtimeType.toString();
  ...
  Widget build(BuildContext context) {
    if (WidgetsApp.debugShowWidgetInspectorOverride || !widget.isEnabled)
      return widget.child;
    ...
    if (isSelectMode && _selections != null && _selections.length > 0)
      children.add(_WidgetDetectorOverlay(
    ...
    return Positioned(
        ...
        child: GestureDetector(
          onPanUpdate: (DragUpdateDetails details) {
            _switchButtonLeft =
                details.globalPosition.dx - _SwitchButton._kWidth / 2;
            _switchButtonTop =
                details.globalPosition.dy - _SwitchButton._kHeight / 2;
            setState(() {});
          },
          onPanEnd: (_) {
            if (_switchButtonLeft + _SwitchButton._kWidth / 2 <
                _windowSize.width / 2)
              _switchButtonLeft = 20;
            else
              _switchButtonLeft =
                  _windowSize.width - 20 - _SwitchButton._kWidth;
            _switchButtonTop = min(
                _windowSize.height - 20 - _SwitchButton._kHeight,
                max(20, _switchButtonTop));
            setState(() {});
          },
          child: _SwitchButton(
              text: isSelectMode ? 'On' : 'Off', onTap: _handleEnableSelect),
        ));
  }
}
...
class _SwitchButton extends StatelessWidget {
  ...
  static const double _kWidth = 80;
...
final Size _windowSize = WidgetsBinding.instance.window.physicalSize /
...
class _WidgetDetectorOverlay extends LeafRenderObjectWidget {
  ...
  _RenderWidgetDetectorOverlay createRenderObject(BuildContext context) {
    return _RenderWidgetDetectorOverlay(
        selections: selections, pointerPosition: pointerPosition);
  }

  @override
  void updateRenderObject(
      BuildContext context, _RenderWidgetDetectorOverlay renderObject) {
...
class _RenderWidgetDetectorOverlay extends RenderBox {
  ...
  static const Color _kHighlightedFillColor =
      Color.fromARGB(128, 128, 128, 255);
  static const Color _kHighlightedBorderColor =
...
//  bool get alwaysNeedsCompositing => true;
      ...
      if (lastRenderBox != null && renderBox.size != lastRenderBox.size) {
        ...
        if (!hasPaintedSelection) {
          ...
          selectionsOfSameSize.forEach((selection) {
            if (selection._isCreatedLocally())
              localCreatedSelections.add(selection);
          });
  ...
  //selections is RenderBoxes  在这里绘制描述文本
  void _paintText(
      Canvas canvas, Rect boundsRect, List<_RenderObjectSelection> selections) {
    ...
    String sizeText =
        "${firstRenderObject.size.width} * ${firstRenderObject.size.width}";
    textSpans.add(TextSpan(
        text: sizeText,
        style: TextStyle(
            color: _kInfoTextColor,
    ...
    if (textPainterSize.width > boundsRect.width ||
        textPainterSize.height > boundsRect.height) {
      //move out of bound rect
      double left =
          min(boundsRect.left, _windowSize.width - textPainterSize.width);
      if (boundsRect.bottom < _windowSize.height * 0.7) {
        ...
        textBgRect = Offset(left, boundsRect.top - textPainterSize.height) &
            textPainterSize;
      } else {
      ...
      textBgRect =
          Offset(boundsRect.left + 1, boundsRect.top) & textPainterSize;
    }
